...
if( args.h ) {
  ...
  ': ' + config.port.name + '----------');
  console.info( 'Reads data object from the device\r');
  console.info( 'Usage: <objectId>\r');
  console.info( '<objectId> identifies the object to read\r');
  process.exit(0);
}
...
port.on( 'open', function () {
  port.master.readObject( args._[0], output );
});
