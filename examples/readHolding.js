...
if( args.h ) {
  ...
  ': ' + config.port.name + '----------');
  console.info( 'Reads holding registers from the device\r');
  console.info( 'Usage: <address> <count>\r');
  console.info( '<address> starting address of register\r');
  console.info( '<count> number of registers to read\r');
  process.exit(0);
}
...
port.on( 'open', function () {
  port.master.readHoldingRegisters( args._[0], args._[1], output );
});
