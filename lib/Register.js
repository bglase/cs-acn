...
Register.prototype.get = function( value ) {
  return this.value;
}
 ...
 *
 */
Register.prototype.valueToBoolArray = function() {
  var b = [];

  var bit = 1;

  for( var i = 0; i < this.length * 16; i++) {
    b.push( (this.value & bit)? true : false );
    bit = bit << 1;
  }
...
Register.prototype.valueToHex16 = function() {
  ...
  return '0x' + this.zeroPad( this.value.toString(16), 4);
};
 ...
 *
 */
Register.prototype.valueToHex32 = function() {
...
Register.prototype.value8ToPercent = function() {
    return Math.max(
      Math.min(
        Math.round((this.value-this.min) * 100 / this.max), 100),0);
};
